...
class CSPNet(nn.Module):
    def __init__(
        ...
        self.mag_gate = mag_gate
        self.mag_zero_mixin = mag_zero_mixin
        assert not (mag_gate and mag_zero_mixin), "mag_gate and mag_zero_mixin are exclusive"
        ...
        if self.cfg:
            if self.mag_gate:
                # Gated DOS conditioning (Flamingo/MatterGen-style mix-in): the
                # [total||m] condition (pred_dim = 2*half) is split into a total
                # pathway (warm-started from the pretrained 400-d y_projection) and
                # a magnetization pathway gated by g = tanh(m_gate). m_gate inits at
                # 0 so g=0 and the m pathway is a no-op -> the model is EXACTLY the
                # pretrained total-DOS model at init, and learns to mix m in.
                assert self.pred_dim % 2 == 0, "mag_gate needs an even pred_dim ([total||m])"
                half = self.pred_dim // 2
                self.y_proj_total = nn.Linear(half, self.hidden_dim)
                self.y_proj_m = nn.Linear(half, self.hidden_dim)
                self.m_gate = nn.Parameter(torch.zeros(1))
                # gate_force (a plain float, NOT a parameter) lets a warmup callback
                # force the effective gate g open for the first N steps so y_proj_m
                # receives gradient (at g=0, dL/dW_m=0 and the channel stays stuck
                # closed). None => use the learned tanh(m_gate).
                self.gate_force = None
                # last-batch projection norms for the contribution-ratio diagnostic
    ...
    def gate_value(self):
        """Effective gate g: the forced warmup value if set, else learned tanh(m_gate)."""
        if self.gate_force is not None:
            return float(self.gate_force)
        return float(torch.tanh(self.m_gate))

    def _project_dos(self, y):
        """Project the DOS condition to hidden_dim. With mag_gate, route the total
        half directly and mix the magnetization half in through g (forced during a
        warmup, else learned = tanh(m_gate)). gate_force is a constant multiplier, so
        y_proj_m still receives gradient through g * y_proj_m(...)."""
        if self.mag_gate:
            half = self.pred_dim // 2
            g = self.gate_force if self.gate_force is not None else torch.tanh(self.m_gate)
            total = self.y_proj_total(y[:, :half])
            gm = g * self.y_proj_m(y[:, half:])
            # stash norms for the contribution-ratio diagnostic ||g*y_proj_m||/||y_proj_total||
            self.last_total_norm = total.detach().norm()
            self.last_gm_norm = gm.detach().norm()
            return total + gm
        if self.mag_zero_mixin:
            half = self.pred_dim // 2
            total = self.y_proj_total(y[:, :half])
            gm = self.y_proj_m(y[:, half:])   # zero at init; self-releases
            self.last_total_norm = total.detach().norm()
            self.last_gm_norm = gm.detach().norm()
            return total + gm
        return self.y_projection(y)
    ...
    def masked_conditional(self, t, atom_types, frac_coords, lattices, num_atoms, node2graph, y=None, mask=None):
        ...
        y = y.to(t_per_atom.dtype)
        y_proj = self._project_dos(y) * mask  # zero out unconditioned atoms after projection
        node_features = node_features + y_proj
