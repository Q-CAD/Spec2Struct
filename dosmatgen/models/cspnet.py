...
class CSPNet(nn.Module):
    def __init__(
        ...
        self.node_out_split = node_out_split
        self.last_total_norm = None   # diagnostic: ||node_out_total(h)|| (last forward)
        self.last_m_norm = None       # diagnostic: ||node_out_m(h)||     (last forward)
        ...
        if self.pred_node_level:
            if self.node_out_split:
                # spin-resolved verifier: per-atom DOS = [total(pred_dim/2) || m(pred_dim/2)].
                # total half is grafted from the released 400-d node_out (byte-identical at
                # init); node_out_m is zero-initialized so m-pred == 0 at init and the
                # backbone stays byte-identical, yet it self-releases (gradient flows from
                # step 0, lift-off distributed across the whole weight matrix). This is the
                # OUTPUT-side application of the zero-init mixin principle (not a tanh gate).
                if pred_dim % 2 != 0:
                    raise ValueError("node_out_split requires an even pred_dim")
                half = pred_dim // 2
                self.node_out_total = nn.Linear(hidden_dim, half, bias=False)
                self.node_out_m = nn.Linear(hidden_dim, half, bias=False)
                nn.init.zeros_(self.node_out_m.weight)
            else:
    ...
    def forward(self, t, atom_types, frac_coords, lattices, num_atoms, node2graph):
        ...
        if self.pred_node_level:
            if self.node_out_split:
                total_out = self.node_out_total(node_features)
                m_out = self.node_out_m(node_features)
                # stash activation norms for the m-contribution ratio (||m||/||total||)
                self.last_total_norm = total_out.detach().norm().item()
                self.last_m_norm = m_out.detach().norm().item()
                node_out = torch.cat([total_out, m_out], dim=-1)
            else:
