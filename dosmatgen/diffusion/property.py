...
class CSPProperty(BaseModule):
    ...
    def configure_optimizers(self):
        """Two-group LR + optional backbone freeze for the spin-resolved verifier.

        Overrides BaseModule's single-group optimizer. Both are config-driven so we can
        compare frozen-backbone vs full fine-tune:
          - optim.freeze_backbone (default False): freeze everything except the zero-init
            m head (decoder.node_out_m.*), shielding the grafted backbone + total head.
          - optim.new_pathway_lr (optional): low LR on backbone + total head, higher LR on
            decoder.node_out_m.* (mirror of the generator's new_pathway_lr).
        """
        optimizer_name = self.hparams.optim.optimizer
        base_params = dict(self.hparams.optim.params)  # lr (=low/inherited), betas, ...

        if self.hparams.optim.get("freeze_backbone", False):
            for n, p in self.named_parameters():
                p.requires_grad = ("decoder.node_out_m." in n)
            print("[optim] freeze_backbone=True -> training only decoder.node_out_m.*")

        new_lr = self.hparams.optim.get("new_pathway_lr", None)
        if new_lr is not None:
            new_p, base_p, new_names, base_names = [], [], [], []
            for n, p in self.named_parameters():
                if not p.requires_grad:
                    continue
                if "decoder.node_out_m." in n:
                    new_p.append(p); new_names.append(n)
                else:
                    base_p.append(p); base_names.append(n)
            assert len(new_p) > 0, "new_pathway_lr set but no decoder.node_out_m params found"
            print(f"[optim] high-LR (lr={new_lr}) group ({len(new_names)}): {new_names}")
            print(f"[optim] low-LR  (lr={base_params.get('lr')}) group: {len(base_names)} params")
            groups = [{"params": base_p}, {"params": new_p, "lr": new_lr}]
            if len(base_p) == 0:            # frozen backbone -> only the m group remains
                groups = [{"params": new_p, "lr": new_lr}]
            optimizer = getattr(torch.optim, optimizer_name)(groups, **base_params)
        else:
            params = [p for p in self.parameters() if p.requires_grad]
            optimizer = getattr(torch.optim, optimizer_name)(params, **base_params)
    ...
    def forward(self, batch):
        ...
        if self.decoder.pred_graph_level:
            ...
            out = {'loss': loss}
            # spin-resolved verifier: report the total / m halves separately. Never a
            # single 800-d L1 — the near-zero m bins hide inside the total magnitude.
            if getattr(self.decoder, 'node_out_split', False):
                half = pred_node.shape[-1] // 2
                out['l1_total'] = F.l1_loss(pred_node[:, :half], batch.y[:, :half])
                out['l1_m'] = F.l1_loss(pred_node[:, half:], batch.y[:, half:])
            return out
        else:
    ...
    def training_step(self, batch: Any, batch_idx: int) -> torch.Tensor:
        ...
        log = {'train_loss': loss}
        if 'l1_total' in output_dict:
            log['train_l1_total'] = output_dict['l1_total']
            log['train_l1_m'] = output_dict['l1_m']
        self.log_dict(
            log,
            on_step=True,
            on_epoch=True,
    ...
    def compute_stats(self, output_dict, prefix):
        ...
        if 'l1_total' in output_dict:
            log_dict[f'{prefix}_l1_total'] = output_dict['l1_total']
            log_dict[f'{prefix}_l1_m'] = output_dict['l1_m']
            # watch the m head lift off zero: activation-norm ratio + weight norm
            tn = getattr(self.decoder, 'last_total_norm', None)
            mn = getattr(self.decoder, 'last_m_norm', None)
            if tn:
                log_dict[f'{prefix}_m_contrib_ratio'] = mn / tn
            log_dict[f'{prefix}_node_out_m_wnorm'] = \
                self.decoder.node_out_m.weight.norm().item()
