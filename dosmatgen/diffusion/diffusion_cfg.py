...
class BaseModule(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        base_params = dict(self.hparams.optim.params)  # lr (=low/inherited LR), betas, ...

        # Optional two-group LR: low LR for inherited weights (backbone + total DOS
        # pathway), higher LR for the freshly-initialized magnetization pathway
        # (decoder.y_proj_m.* + decoder.m_gate). Enabled by optim.new_pathway_lr.
        new_lr = self.hparams.optim.get("new_pathway_lr", None)
        if new_lr is not None:
            new_names, base_names, new_p, base_p = [], [], [], []
            for n, p in self.named_parameters():
                if not p.requires_grad:
                    continue
                if ("decoder.y_proj_m." in n) or (n == "decoder.m_gate") or n.endswith(".m_gate"):
                    new_p.append(p); new_names.append(n)
                else:
                    base_p.append(p); base_names.append(n)
            print(f"[optim] high-LR (lr={new_lr}) group ({len(new_names)} params): {new_names}")
            print(f"[optim] low-LR  (lr={base_params.get('lr')}) group: {len(base_names)} params "
                  f"(e.g. {base_names[:3]} ... {base_names[-2:]})")
            assert len(new_p) > 0, "new_pathway_lr set but no y_proj_m/m_gate params found"
            optimizer = getattr(torch.optim, optimizer_name)(
                [{"params": base_p}, {"params": new_p, "lr": new_lr}], **base_params
            )
        else:
            optimizer = getattr(torch.optim, optimizer_name)(
...
class CSPDiffusion(BaseModule):
    ...
    def training_step(self, batch: Any, batch_idx: int) -> torch.Tensor:
        ...
        if hasattr(self.decoder, "m_gate"):
            self.log("m_gate_g", float(self.decoder.gate_value()),
                     on_step=False, on_epoch=True, prog_bar=True)
        # contrib ratio applies to both m pathways (tanh-gated and zero-init mixin)
        tn = getattr(self.decoder, "last_total_norm", None)
        gn = getattr(self.decoder, "last_gm_norm", None)
        if tn is not None and gn is not None and float(tn) > 0:
            self.log("m_contrib_ratio", float(gn) / float(tn),
                     on_step=False, on_epoch=True, prog_bar=True)
        # weight norm of the zero-init m projection: the lift-off diagnostic
        if getattr(self.decoder, "mag_zero_mixin", False):
            self.log("y_proj_m_wnorm", self.decoder.y_proj_m.weight.norm().item(),
                     on_step=False, on_epoch=True, prog_bar=True)
    ...
    @torch.no_grad()
    def template_cfg_sample(self, batch, pin_mask, step_lr=1e-5, w=1.0):
        """Defect-template CFG generation.

        pin_mask: bool [num_nodes]. PINNED atoms carry host truth: their
        batch.frac_coords / batch.atom_types are held fixed at every denoising
        step (fix_sample's node-0 pattern generalized to arbitrary per-node
        masks, which also fixes its batch>1 indexing bug). The lattice is fixed
        to the batch cell (host cell) throughout. FREE atoms start from PURE
        noise -- no diff_ratio warm-start leakage anywhere. Per-atom DOS
        conditioning (batch.y) applies to ALL atoms via the standard
        cond/uncond CFG pair: pred = (1+w)*cond - w*uncond.
        """
        batch_size = batch.num_graphs
        pin = pin_mask.bool().view(-1)

        l_T = lattice_params_to_matrix_torch(batch.lengths, batch.angles)
        x_T = torch.rand([batch.num_nodes, 3]).to(self.device)
        t_T = torch.randn([batch.num_nodes, MAX_ATOMIC_NUM]).to(self.device)
        pin_x = batch.frac_coords[pin]
        pin_t = F.one_hot(batch.atom_types - 1,
                          num_classes=MAX_ATOMIC_NUM).float()[pin]
        x_T[pin] = pin_x
        t_T[pin] = pin_t
        ...
        for t in tqdm(range(time_start, 0, -1)):
            ...
            def cfg_eps(t_in, x_in):
                un_x, un_l, un_t, _, _ = self.decoder(
                    time_emb, t_in, x_in, l_t, batch.num_atoms, batch.batch,
                    batch.y, unconditional=True, conditional=False)
                co_x, co_l, co_t, _, _ = self.decoder(
                    time_emb, t_in, x_in, l_t, batch.num_atoms, batch.batch,
                    batch.y, unconditional=False, conditional=True)
                return ((1 + w) * co_x - w * un_x,
                        (1 + w) * co_t - w * un_t)
            ...
            step_size = step_lr * (sigma_x / self.sigma_scheduler.sigma_begin) ** 2
            std_x = torch.sqrt(2 * step_size)
            pred_x, _ = cfg_eps(t_t, x_t)
            pred_x = pred_x * torch.sqrt(sigma_norm)
            x_t_minus_05 = x_t - step_size * pred_x + std_x * rand_x
            x_t_minus_05[pin] = pin_x
            t_t_minus_05 = t_t
            ...
            std_x = torch.sqrt((adjacent_sigma_x ** 2
                               ...
                               / (sigma_x ** 2))
            pred_x, pred_t = cfg_eps(t_t_minus_05, x_t_minus_05)
            pred_x = pred_x * torch.sqrt(sigma_norm)
            x_t_minus_1 = x_t_minus_05 - step_size * pred_x + std_x * rand_x
            t_t_minus_1 = c0 * (t_t_minus_05 - c1 * pred_t) + sigmas * rand_t
            x_t_minus_1[pin] = pin_x
            t_t_minus_1[pin] = pin_t
